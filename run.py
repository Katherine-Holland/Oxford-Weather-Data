...
def delete_weather_data(month):
    ...
    attempts = 0
    while attempts < 2:
        confirm = input(f"Are you sure you want to delete the data for {MONTH_NAMES[month-1]} 2024? (yes/no): ").strip().lower()
        if confirm == 'yes':
            try:
                ...
                print(f"Data successfully deleted for {MONTH_NAMES[month-1]} 2024.")
